...
if [[ "${VSCODE_ARCH}" == "x64" ]]; then
  ...
  # Glider - this line is important - ask GPT about it
  sed -i 's/generate_type2_appimage/generate_type2_appimage -u "gh-releases-zsync|GliderOrg|binaries|latest|*.AppImage.zsync"/' pkg2appimage.AppDir/AppRun

  # remove check so build in docker can succeed
  sed -i 's/grep docker/# grep docker/' pkg2appimage.AppDir/usr/share/pkg2appimage/functions.sh

  if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
    sed -i 's|@@NAME@@|Glider-Insiders|g' recipe.yml
    sed -i 's|@@APPNAME@@|glider-insiders|g' recipe.yml
    sed -i 's|@@ICON@@|glider-insiders|g' recipe.yml
  else
    # Glider branding instead of default codium
    sed -i 's|@@NAME@@|Glider|g' recipe.yml
    sed -i 's|@@APPNAME@@|glider|g' recipe.yml
    sed -i 's|@@ICON@@|glider|g' recipe.yml
  fi
  ...
  rm -rf Glider* # Glider had this commented out at one point
fi
