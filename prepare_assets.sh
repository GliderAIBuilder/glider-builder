...
if [[ "${OS_NAME}" == "osx" ]]; then
  ...
  #  # TODO: Enable code signing for macos app verification
  #  #npx create-dmg ./*.app .
  #  npx create-dmg ./*.app . --overwrite --no-code-sign
  #  mv ./*.dmg "../assets/${APP_NAME}.${VSCODE_ARCH}.${RELEASE_VERSION}.dmg"
  #  popd
  #fi

  if [[ "${SHOULD_BUILD_DMG}" != "no" ]]; then
    echo "Building and moving DMG using hdiutil (no code signing)"
    pushd "VSCode-darwin-${VSCODE_ARCH}"
  
    APP_BUNDLE=$(echo ./*.app)
    DMG_NAME="${APP_NAME}.${VSCODE_ARCH}.${RELEASE_VERSION}.dmg"
    VOLUME_NAME="${APP_NAME} Installer"
  
    if [[ ! -d "${APP_BUNDLE}" ]]; then
      echo "❌ No .app bundle found in $(pwd)"
      exit 1
    fi
